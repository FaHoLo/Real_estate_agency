...
class Flat(models.Model):
    ...
    new_building = models.NullBooleanField("Новостройка ли", db_index=True)
...
class Owner(models.Model):
    ...
    phone_pure = PhoneNumberField(verbose_name="Нормализованный номер владельца", blank=True)
    flats = models.ManyToManyField(Flat, verbose_name="Квартиры в собственности", related_name='owners')

    def __str__(self):
        return f"{self.name}, {self.phone_pure}"
